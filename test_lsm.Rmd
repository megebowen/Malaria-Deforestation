...
##load the Altamira tile. CREATE INTO A RASTER BRICK?
...
year_fxn <- function(year) {
  alta_yr <- alta_clip %in% 1:year
  return(alta_yr)
}
...
#for loop to separate out by "nome"
##this is almost it! I only have "Guimares" as an output rn. 
landscape_output <- vector("character", length = ncol(muni_shp))

for(i in muni_shp$nome) {
  landscape_output <- muni_shp %>% 
    dplyr::filter(nome == i)
}
