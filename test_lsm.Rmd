...
alta_extract <- extract(alta_lossyear, altamira_shp,
                        method = 'simple',
                        fun = max,
                        sp = T)
...
#function
year_rast <- function(alta_mask) {
  alta_i <- alta_mask %in% 1:alta_values
...
for (i in 1:total_poly) {
  ...
  municrop <- crop(alta_lossyear, extent(muni_spd[i,])) ##need to change to all lossyear raster
  
  ##mask with above cropped extent (i) for EACH polygon in muni SpatialDataFrame (i)
  munimask <- mask(municrop[i,], muni_spd[i,])##test if this works
}
