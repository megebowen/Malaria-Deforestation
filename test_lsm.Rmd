...
lsm_te <- lsm_l_te(alta_2001)
lsm_ed <- lsm_l_ed(alta_2001) %>% 
  dplyr::select(metric, value) %>% 
  rename(metric_ed = metric, value_ed = value)
lsm_area <- lsm_l_area_mn(alta_2001)
lsm_ta <- lsm_l_ta(alta_2001, directions = 8)

lsm_all <- cbind(alta_spd$codigo_ibg, lsm_te, lsm_ed) %>% 
  mutate(year = 2001)

lsm_te2 <- lsm_l_te(alta_2002)
lsm_ed2 <- lsm_l_ed(alta_2002) 
lsm_area2 <- lsm_l_area_mn(alta_2002)
lsm_ta2 <- lsm_l_ta(alta_2002, directions = 8)

lsm_all2 <- rbind(lsm_te2, lsm_ed2, lsm_area2, lsm_ta2) %>% 
  mutate(year = 2002)

lsm_upd <- rbind(lsm_all, lsm_all2)

lsm_02 <- lsm_l_te(alta_2002) %>% 
  dplyr::mutate(year="2002")
lsm_03 <- lsm_l_te(alta_2003) %>% 
...
- mean patch size: interesting for loss in the year (mean patch size of forest loss) and total forest loss :: lsm_l_area_mn
- edge density :: lsm_l_ed
- total cover ::lsm_l_ta (directions = 8)
- patch clumpiness: is loss happening in distinct patterns? or randomly across the landscape? ::lsm_c_clumpy
