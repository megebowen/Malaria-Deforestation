...
##test: cut down file size

all_rast <- raster("lossyear_arc_all.tif")

muni_X <- all_rast %in% 1
clip1 <- crop(muni_X, extent(codigo_spd)) #crop to polygon extent (mask will go faster) ##CHANGE YEAR
clip2 <- mask(clip1, codigo_spd) %>% #mask polygon to above cropped area (takes less time)
  writeRaster(filename = "muni_2001_NEWTEST.tif", format = "GTiff")
